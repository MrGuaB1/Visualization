...
async function fetchData() {
    try {
        const response = await fetch('http://localhost:8080/data/exports');
        if (!response.ok) {
...
// 创建图表
async function create_export_chart() {
    await fetchData();
    ...
    function showCard(event, d) {
        const card = d3.select("#infoCard_export");
        // 更新卡片内容
        card.select(".card-title")
            .text(d.data.name);
        card.select(".card-value")
            .text(`出口量（吨）: ${d.data.value}`);
        // 设置卡片位置
...
// 关闭卡片
function closeCard_export() {
    d3.select("#infoCard_export").style("visibility", "hidden");
}

// 隐藏卡片
function hideCard_export() {
    d3.select("#infoCard_export").style("visibility", "hidden");
}